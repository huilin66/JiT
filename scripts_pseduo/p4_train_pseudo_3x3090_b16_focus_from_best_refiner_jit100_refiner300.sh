...
# P4 3x3090 experiment:
#   Stage 1: lightly fine-tune JiT-B/16 on focus_2scene pseudo data from the
#            best historical B16 focus_2scene refiner checkpoint.
#   Stage 2: train a frozen-JiT MSDT detail refiner from Stage-1 checkpoint-last.
#
# Motivation:
#   P2 retrained dn_blur4 JiT-B from the generic JiT-B checkpoint and scored low
#   with incomplete raindrop removal. This run keeps the strongest focus2scene
#   prior and only adapts it to the pseudo-augmented dataset.
#
# Best-history anchor:
#   score=34.4282
...
GPUS=${GPUS:-0,1,2}
NPROC=${NPROC:-3}
MASTER_PORT_JIT=${MASTER_PORT_JIT:-30640}
MASTER_PORT_REFINER=${MASTER_PORT_REFINER:-30641}
...
INIT_CKPT=${INIT_CKPT:-run/train/b16_focus_2scene_msdt_refiner_plan_c12_01_c1_higher_than_c_1x5090/16}
INIT_STATE_KEY=${INIT_STATE_KEY:-model_ema1}
SCENE_FOCUS_2_PATH=${SCENE_FOCUS_2_PATH:-${DATA_PATH}/Drop_focus_2scene_test_pseudo.json}
OUT_ROOT=${OUT_ROOT:-run/train_pseudo}
JIT_OUTPUT_DIR=${JIT_OUTPUT_DIR:-${OUT_ROOT}/p4_b16_focus_2scene_from_best_refiner_jit_ft_100ep_3x3090/16}
REFINER_OUTPUT_DIR=${REFINER_OUTPUT_DIR:-${OUT_ROOT}/p4_b16_focus_2scene_from_best_refiner_refiner_300ep_3x3090/16}
...
JIT_LR=${JIT_LR:-2.5e-5}
JIT_EPOCHS=${JIT_EPOCHS:-100}
...
echo "============================================================"
echo "[P4 pseudo 3x3090] B16 focus2scene from best refiner: JIT 100ep + refiner 300ep"
echo "RUN_STAGES=${RUN_STAGES}"
echo "DATA_PATH=${DATA_PATH}"
echo "SCENE_FOCUS_2_PATH=${SCENE_FOCUS_2_PATH}"
echo "GPUS=${GPUS}, nproc=${NPROC}"
echo "JIT: init=${INIT_CKPT}, state=${INIT_STATE_KEY}, output=${JIT_OUTPUT_DIR}, batch=${JIT_BATCH_SIZE}, lr=${JIT_LR}, epochs=${JIT_EPOCHS}"
...
if has_stage "JIT"; then
  echo "============================================================"
  echo "[P4 Stage 1] Fine-tune JiT-B/16 focus2scene from best refiner checkpoint"
  echo "============================================================"
  CUDA_VISIBLE_DEVICES="${GPUS}" torchrun \
    ...
    --resume "${INIT_CKPT}" \
    --resume_state_key "${INIT_STATE_KEY}" \
...
if has_stage "REFINER"; then
  ...
  echo "============================================================"
  echo "[P4 Stage 2] Train MSDT refiner from focus2scene JiT-B last"
  echo "============================================================"
  CUDA_VISIBLE_DEVICES="${GPUS}" torchrun \
    --nproc_per_node="${NPROC}" \
...
echo "P4 pseudo B16 focus2scene JIT/refiner training finished."
