...
# Train larger focus-2scene JiT backbones on 3 x RTX 3090.
# Defaults are conservative for 24GB cards:
#   JiT-L/16: batch 4 per GPU
#   JiT-H/16: batch 2 per GPU
#
# Examples:
#   RUN_MODELS="L" bash scripts/train_focus_2scene_jit_lh_3x3090.sh
#   CKPT_L=ckpt/jit-l-16 CKPT_H=ckpt/jit-h-16 RUN_MODELS="L H" bash scripts/train_focus_2scene_jit_lh_3x3090.sh
...
GPUS=${GPUS:-0,1,2}
NPROC=${NPROC:-3}
MASTER_PORT_BASE=${MASTER_PORT_BASE:-30120}
...
OUT_ROOT=${OUT_ROOT:-run/train/focus_2scene_lh_no_head_3x3090}
...
run_model() {
  ...
  echo "============================================================"
  echo "[JiT-${tag}] focus 2scene no-head on 3x3090"
  echo "model=${model}"
  echo "ckpt=${ckpt}"
...
for model_tag in ${RUN_MODELS}; do
  case "${model_tag}" in
    L|l)
      run_model "l16" "JiT-L/16" "${CKPT_L}" "${BATCH_SIZE_L}" "${LR_L}" "$((MASTER_PORT_BASE + 0))"
      ;;
    H|h)
      run_model "h16" "JiT-H/16" "${CKPT_H}" "${BATCH_SIZE_H}" "${LR_H}" "$((MASTER_PORT_BASE + 1))"
      ;;
    *)
...
echo "Requested JiT-L/H focus-2scene trainings finished: ${RUN_MODELS}"
