...
class DynamicRaindropLoss(nn.Module):
    def __init__(self, device, target_w_rec=1.0, target_w_ssim=1.0, target_w_lpips=0.5, total_epochs=200):
        super().__init__()
    ...
    def update_weights(self, current_epoch):
        ...
        if current_epoch < warmup_epochs:
            ...
            progress = (current_epoch - warmup_epochs) / (self.total_epochs - warmup_epochs)
    ...
    def forward(self, pred, target):
        ...
        loss_ssim = 1.0 - ssim(pred_01.float(), target_01.float(), data_range=1.0, size_average=True)
        ...
        total_loss = (self.cur_w_rec * loss_rec) + \
