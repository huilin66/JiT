...
class EnhancedInferencer:
    ...
    def forward(self, img_tensor, step_num, dummy_labels1, dummy_labels2):
        """
        ...
        dummy_labels_list = [dummy_labels1, dummy_labels2]
        # 遍历所有模型 (Model Ensembling)
        for model, model_w, dummy_labels in zip(self.models, self.model_weights, dummy_labels_list):
...
def ensemble_predict(device=0, aug=False, model_weights=[0.5, 0.5], use_scene_dataset=True):
    device = torch.device(f"cuda:{device}" if torch.cuda.is_available() else "cpu")
    print(f"🔥 Starting Multi-Model Ensemble Inference on {device}...")
    ...
    save_dir = f"{data_root}/submission_test-input/ensemble_step{step_num}_last_{use_scene_dataset}"
    ...
    if aug:
        inferencer = EnhancedInferencer(
            models=[model_H, model_B],  # 💥 传入模型列表
            model_weights=model_weights,  # 💥 赋予 H 模型 75% 的权重，B 模型 25% 的权重
            use_h_flip=True,  # 开启 TTA
            scales=[0.8, 1.0, 1.25, 2.0],  # 时间如果够可以开 [0.8, 1.0, 1.25]
            use_color_match=True,
            patch_size=256,
            stride=128,
        )
        ...
        inferencer = EnhancedInferencer(
            models=[model_H, model_B],  # 💥 传入模型列表
            model_weights=model_weights,  # 💥 赋予 H 模型 75% 的权重，B 模型 25% 的权重
            use_h_flip=False,  # 开启 TTA
            scales=[1.0],  # 时间如果够可以开 [0.8, 1.0, 1.25]
    ...
    for img_name in tqdm(files, desc="predict"):
        ...
        scene_id = scene_dict[img_name]
        dummy_labels1 = torch.zeros(1, dtype=torch.long, device=device) + scene_id
        dummy_labels2 = None

        # 直接调用强大的推理器
        final_tensor = inferencer.forward(img_tensor, step_num, dummy_labels1, dummy_labels2)
...
if __name__ == "__main__":
    # ensemble_predict(aug=True, model_weights=[0.5, 0.5])
    ensemble_predict(aug=True, model_weights=[0.25, 0.75])
    # ensemble_predict(aug=True, model_weights=[0.75, 0.25])
